...
async function checkExistingSetup() {
  try {
    const manifest = JSON.parse(
      await fs.readFile("manifest.json.json", "utf8")
    );
    const packageJson = JSON.parse(await fs.readFile("package.json", "utf8"));

    // Check if manifest has been customized (not using default values)
    const isManifestCustomized =
      manifest.id !== "obsidian-svelte5-template" ||
      manifest.name !== "Obsidian Svelte 5 Template" ||
      manifest.description !==
        "A template for building Obsidian plugins with Svelte 5" ||
      manifest.author !== "Steven Stavrakis";

    // Check if package name has been changed from default
    const isPackageCustomized =
      packageJson.name !== "obsidian-svelte5-template";

    if (isManifestCustomized || isPackageCustomized) {
      log.warn("⚠️  WARNING: Existing Configuration Detected ⚠️");
      log.warn(
        "It appears this plugin has already been set up or manually configured."
      );
      log.warn(
        "Running this script will overwrite any existing customizations."
      );
...
This script will help you configure your new Obsidian plugin`);
  ...
  const promptGroup = await group(
    {
      displayName: () =>
        text({
          ...
          validate: (value) => {
            if (value.length === 0)
              return "Please enter a name for your plugin";
            if (value.length < 3)
              return "Plugin name must be at least 3 characters long";
          },
        }),
      ...
      description: () =>
        text({
          ...
          validate: (value) => {
            if (value.length === 0)
              return "Please provide a description of your plugin";
            if (value.length < 10)
      ...
      authorUrl: () =>
        text({
          ...
          validate: (value) => {
            if (value.length === 0)
              return "Please provide your website or GitHub URL";
  ...
  await fs.writeFile("manifest.json.json", JSON.stringify(manifest, null, 2));
  ...
  log.warn(
    "Please restart Obsidian and re-activate any plugins in the dev-vault"
  );
